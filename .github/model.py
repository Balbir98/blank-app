...
if uploaded_file:
    ...
    st.write(f"Total advisers available for modelling: {n_samples}")
    
    if n_samples < 10:
        st.error("Not enough data to build reliable models (need at least 10 records).")
    else:
        # Determine test_size dynamically to ensure at least one test sample
        ts = 0.2 if n_samples * 0.2 >= 1 else 1
        ...
        try:
            X_train, X_test, y_train, y_test = train_test_split(
                X, y_reg, test_size=ts, random_state=42
            )
            ...
            importances_reg = pd.Series(
                reg.feature_importances_, index=feature_cols
        ...
        df_clean['Underperformer'] = (
            df_clean['Total Commission Earned Year To Date'] <= 
            df_clean['Total Commission Earned Last Year']
        ).astype(int)
        ...
        try:
            X2_train, X2_test, y2_train, y2_test = train_test_split(
                X2, y_clf, test_size=ts, random_state=42
            )
